...
if not os.path.isfile(parserargs["refroi"]):
    raise ValueError(
        "The refroi " + parserargs["refroi"],
        " does not exists, did you create a normalization ROI using freeview already?",
    )
...
for imagepath in images:
    ...
    assert numpy.allclose(
        refroi_affine, refroi_affine
    ), "Affine transformations differ, are you sure the images are registered properly?"

    nibabel.save(
        nibabel.Nifti1Image(normalized_image, refroi_affine),
        exportfolder / imagepath.name,
    )
