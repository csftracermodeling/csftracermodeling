...
if __name__ == "__main__":
    ...
    pinn_images = [
        pathlib.Path(parserargs["pinnfolder"]) / x
        for x in os.listdir(pathlib.Path(parserargs["pinnfolder"]))
        if x.endswith(".mgz")
    ]

    if len(pinn_images) == 0:
        raise ValueError(
            "pinn-postprocess.py need to be run on ",
            parserargs["pinnfolder"],
            "first to create .mgz images",
        )

    fem_images = [
        pathlib.Path(parserargs["femfolder"]) / x
        for x in os.listdir(pathlib.Path(parserargs["femfolder"]))
        if x.endswith(".mgz")
    ]

    if len(fem_images) == 0:
        raise ValueError(
            "fem-postprocess.py need to be run on ",
            parserargs["femfolder"],
            "first to create .mgz images",
        )
    ...
    data = Voxel_Data(
        ...
        mask=mask,
        pixelsizes=[1, 1, 1],
        Tmax=pinn_hyperparameters["Tmax"],
        verbosity=0,
    )
    ...
    for name, image in zip(names, images):
        ...
        try:
            ...
            print(
                "Some modules for plotting could not be imported, will not zoom to data"
            )
        ...
        if name != "MRI":
            print(
                "l2 difference MRI-",
                name,
                format(np.nansum((mri[mask] - image[mask]) ** 2), ".2e")
                + " at t="
                + format(data.measurement_times()[time_idx], ".0f")
                + " hours",
            )

            if np.sum(np.isnan(image[mask])) > 0.1 * image[mask].size:
                raise ValueError(
                    "More than 10 % nans in image, probably something wrong"
                )
