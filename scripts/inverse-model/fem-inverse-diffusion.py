...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data",
        default="./data/mridata3d/CONCENTRATIONS/",
        help=(
            "Path to folder containing concentrations in mgz format."
            "Assuming that imaging time is contained as YYYYMMDD_HHMMSS' in filename."
        ),
    )
    parser.add_argument(
        "--mesh",
        default="./roi12/parenchyma_mask_roi12.xml",
        help="path to mesh as xml file.",
    )
    parser.add_argument(
        "--mask",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "The script exists without performing any simulation or writing files."
        ),
    )
    parser.add_argument("--lbfgs_iters", default=42)
    parser.add_argument(
        "--dt", default=3600, type=float, help="timestep size in seconds"
    )
    parser.add_argument(
        "--taylortest",
        default=False,
        action="store_true",
        help="Run Taylor test to check if gradients are correct.",
    )
    ...
    if outfolder.is_dir():
        if parserargs["overwrite_output"] is True:
            shutil.rmtree(outfolder)

        else:
            print("Output folder ", outfolder, "exists, did not delete it.")
            exit()
    ...
    assert meshpath.is_file() is True
    ...
    print(
        "MeshQuality.radius_ratio_min_max=", MeshQuality.radius_ratio_min_max(brainmesh)
    )

    assert (
        min(MeshQuality.radius_ratio_min_max(brainmesh)) > 1e-6
    ), "Mesh contains degenerated cells"
    ...
    diffusion_model.forward(
        water_diffusivity=mean_water_diffusivity,
        r=reaction_rate,
        taylortest=parserargs["taylortest"],
    )
    ...
    if parserargs["taylortest"]:
        conv_rate = taylor_test(
            J=Jhat, m=[Constant(1e-2), Constant(1e-5)], h=[Constant(0), Constant(1e-5)]
        )
        print(conv_rate)
        conv_rate = taylor_test(
            J=Jhat, m=[Constant(1e-2), Constant(1e-5)], h=[Constant(1e-2), Constant(0)]
        )
        print(conv_rate)
        print("convergence test done, exiting script.")
    ...
    vals = numpy.zeros(1)
    vals[0] = assemble(opt_ctrls[0] * dx(domain=brainmesh)) / assemble(
        1 * dx(domain=brainmesh)
    )
    print("After optim. D (water)      =", format(vals[0], ".2e"))
    print(
        "After optim. D (gadobutrol) =",
        format(vals[0] * diffusion_model.scale_diffusion_gad, ".2e"),
    )
    ...
    vals = numpy.zeros(1)
    vals[0] = assemble(opt_ctrls[1] * dx(domain=brainmesh)) / assemble(
        1 * dx(domain=brainmesh)
    )
    ...
    L2_mismatch = diffusion_model.forward(
        water_diffusivity=final_D, r=final_r, taylortest=parserargs["taylortest"]
    )

    print("Simulation done, mismatch=", format(L2_mismatch, ".2f"))

    print(
        "Final / Initial Mismatch =",
        format(float(L2_mismatch) / initial_missmatch, ".2f"),
    )

    if float(L2_mismatch) / initial_missmatch > 1:
        raise ValueError("Optimization worsened the mismatch between model and data.")
