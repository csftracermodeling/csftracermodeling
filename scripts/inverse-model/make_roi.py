...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--maskfile",
        default="./data/freesurfer/mri/parenchyma_mask.mgz",
    ...
    parser.add_argument(
        "-i", "--center_i", type=int, default=146, help="roi center in first direction"
    )
    parser.add_argument(
        "-j", "--center_j", type=int, default=100, help="roi center in first direction"
    )
    parser.add_argument(
        "-k", "--center_k", type=int, default=133, help="roi center in first direction"
    )
    ...
    if outfolder.is_dir():
        ...
        while answer not in ["y", "n"]:
            answer = input(
                "Outputfolder "
                + str(outfolder)
                + "  exists, delete and create files from scratch? [y, n]:"
            ).lower()
    ...
    for di, dj, dk in itertools.product(range(-radius, radius + 1), repeat=3):
        if abs(di) ** 2 + abs(dj) ** 2 + abs(dk) ** 2 <= radius**2:
            roi[i_cursor + di, j_cursor + dj, k_cursor + dk] = 1.0
    ...
    cmd = "meshio convert " + outfile + " " + outfile.replace(".mesh", ".xml")
    subprocess(cmd, shell=True).check_returncode()
    cmd = "meshio convert " + outfile + " " + outfile.replace(".mesh", ".xdmf")
    subprocess(cmd, shell=True).check_returncode()
    """
    ...
    try:
        ...
        print(
            "MeshQuality.radius_ratio_min_max=",
            MeshQuality.radius_ratio_min_max(roimesh),
        )

        assert (
            min(MeshQuality.radius_ratio_min_max(roimesh)) > 1e-6
