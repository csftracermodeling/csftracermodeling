...
if __name__ == "__main__":
    ...
    parser.add_argument("--maskfile", default="./data/freesurfer/mri/parenchyma_mask.mgz",
                        help="Path to parenchyma_mask.mgz")
    parser.add_argument("--resolution", type=int, default=20,
                        help="Resolution of the volume mesh for the ROI")
    parser.add_argument("-R", "--radius", type=int, default=10,
                        help="ROI radius in number of voxels (integer)")
    parser.add_argument("-i", "--center_i", type=int,
                        default=146, help="roi center in first direction")
    parser.add_argument("-j", "--center_j", type=int,
                        default=100, help="roi center in first direction")
    parser.add_argument("-k", "--center_k", type=int,
                        default=133, help="roi center in first direction")
    ...
    if outfolder.is_dir():
        ...
        while answer not in ["y", "n"]:
            answer = input("Outputfolder " + str(outfolder) +
                           "  exists, delete and create files from scratch? [y, n]:").lower()

        if not answer == "y":
            print("Not deleting outputfolder" +
                  str(outfolder) + " (not recommended).")
        else:
    ...
    try:
        ...
        boundaryfile = str(
            outfolder / maskfile.replace(".mgz", "_boundary.mgz"))
        nibabel.save(nibabel.Nifti1Image(
            boundary, mask_volume.affine), boundaryfile)
    ...
    os.system("mri_binarize --i " + roifile +
              " --match 1 --surf-smooth 3 --surf " + roisurffile)
    ...
    try:
        ...
        print("Number of vertices =", format(
            roimesh.coordinates().shape[0], ".1e"))
        print("MeshQuality.radius_ratio_min_max=",
              MeshQuality.radius_ratio_min_max(roimesh))

        assert min(MeshQuality.radius_ratio_min_max(roimesh)
                   ) > 1e-6, "Mesh contains degenerated cells"
    except ModuleNotFoundError:
        print()
