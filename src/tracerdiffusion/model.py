...
class Model(object):
    def __init__(
        ...
        self.mri_times = [
            x * 3600 for x in mris.measurement_times()
        ]  # simulation scripts work in units of hours
        self.T = max(self.mri_times)

        assert (
            max(mris.measurement_times()) <= 6 * 24
        ), "MRI times should be in hours, maybe something is wrong?"
        ...
        print(
            ...
            "(",
            format(self.T / 1, ".0f"),
            "seconds)" " with time step size",
            format(self.dt / 60, ".0f"),
            "minutes",
        )
        ...
        for mri_time in self.mri_times:
            print(
                "Data available at",
                format(mri_time / 3600, ".0f"),
                "hours past first image",
            )
            self.checkpoints.append(
                np.round(times[np.argmin(np.abs(times - mri_time))], 0).item()
            )
    ...
    def save_predictions(self, name="simulation"):
        ...
        hdf5file = HDF5File(
            self.V.mesh().mpi_comm(), str(self.outfolder / (name + ".hdf")), "w"
        )
        ...
        checkpoints = {"simulation": self.checkpoints, "data": self.mri_times}
    ...
    def advance_time(self, current_state):
        ...
        if self.t > self.mri_times[self.image_counter]:
            ...
            while self.t > self.mri_times[self.image_counter]:
                if self.verbosity == 1:
                    print(
                        "t=",
                        format(self.t / 3600, ".2f"),
                        "Increasing image counter from",
                        self.image_counter,
                        "to",
        ...
        if np.round(self.t, 0) in self.checkpoints:
            dt = (
                self.t
                - self.mri_times[np.argmin(np.abs(np.array(self.checkpoints) - self.t))]
            )
            ...
            if config.inverse:
                self.simulated_tracer.append(
                    current_state.copy(deepcopy=True, annotate=False)
                )
            else:
            ...
            if self.verbosity == 0:
                print(
                    "Rel. L2 error ||c-cdata|| / ||cdata|| at t=",
                    format(self.t / 3600, ".2f"),
                    "is",
                    format(L2_error / datanorm, ".2f"),
                    "(image ",
                    i + 1,
                    "/",
                    len(self.data),
    ...
    def boundary_condition(self):
        ...
        try:
            ...
            if self.verbosity == 1:
                print(
                    "time=",
                    format(self.t / 3600, ".0f"),
                    "h, image_counter=",
                    self.image_counter,
                )

            self.linear_interpolation.vector()[:] = self.data[
                self.image_counter_prev
            ].vector()[:]

            finite_difference = (
                self.data[self.image_counter].vector()[:]
                - self.data[self.image_counter_prev].vector()[:]
            )
            finite_difference /= (
                self.mri_times[self.image_counter]
                - self.mri_times[self.image_counter_prev]
            )

            self.linear_interpolation.vector()[:] += finite_difference * (
                self.t - self.mri_times[self.image_counter_prev]
            )

            if config.inverse:
                self.bcs[self.t] = DirichletBC(
                    self.V,
                    self.linear_interpolation.copy(deepcopy=True, annotate=False),
                    boundary,
                )
            else:
                self.bcs[self.t] = DirichletBC(
                    self.V, self.linear_interpolation.copy(deepcopy=True), boundary
                )
    ...
    def forward(
        self, water_diffusivity, diffusion_tensor_water=None, r=None, taylortest=False
    ):
        self.L2_error = 0.0
        self.datanorm = 0.0
        self.bcs = {}
        ...
        print(
            "Using D = "
            + format((self.scale_diffusion_gad * vals)[0], ".2e")
            + " mm^2/s for gadobutrol diffusion (cf. Valnes et al Scientific Reports 2020 for details)"
        )
        ...
        movie_hdf = HDF5File(
            self.V.mesh().mpi_comm(), str(self.outfolder / "movie.hdf"), "w"
        )
        ...
        def diffusion(fun):
            if self.dx_SD is not None:
                ...
                if diffusion_tensor is not None:
                    ...
                    term += inner(
                        dot(diffusion_tensor, grad(fun)), grad(v)
                    ) * self.dx_SD(2)
                else:
                    # use mean diffusivity in white matter
                    term += (
                        self.mean_diffusivity
                        * inner(grad(fun), grad(v))
                        * self.dx_SD(2)
                    )

                # brain stem:
                term += (
                    self.mean_diffusivity * inner(grad(fun), grad(v)) * self.dx_SD(3)
                )
