...
# integrators
interior_integrator = EvolutionaryIntegrator(interior, key=random.PRNGKey(seed), N=10000)
...
def model_d(params_d, x):
        w, b = params_d[0]
        d = jax.nn.sigmoid(w + 0. * b) + jnp.array([0.000001])
...
def heat_operator(u, params_d):
    return lambda tx: dt(u)(tx) - model_d(params_d, jnp.array([0.])) * (ddx_1(u)(tx) + ddx_2(u)(tx))
