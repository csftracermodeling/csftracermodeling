...
def get_delta_t(file1: str, file2 : str, file_suffix="", name_format = None) -> float:
    ...
    if name_format is not None:
        ...
        def time_from_filename(file):
            ...
            if parser_result is None or parser_result[0] is None:
                raise ValueError("Could not infer time in hours from file name. Expecting file name to be of form <12.34.mgz> or <1.23.mgz>")
    ...
    if time > TMAX_HOURS:
        raise ValueError(str(time) + "> TMAX_HOURS=" + str(TMAX_HOURS) + ", something wrong in file name conversion?")
...
class Nan_Filter():
    ...
    def __init__(self, mask):
        ...
        for x, y, z in itertools.product(range(- filter_size, filter_size + 1), repeat=3):
...
class MRI_Data(abc.ABC):
    ...
    def __init__(self, datapath, Tmax = 4 * 24, verbosity: int = 1):
        ...
        for filename in self.files:
            ...
            if verbosity == 1:
                print("Key=", key, "corresponds to image=", self.time_filename_mapping[key], "in data.time_filename_mapping")
...
class Voxel_Data(MRI_Data):
    ...
    def __init__(self, mask = None, pixelsizes: list = [1,1,1], *args, **kwargs) -> None:
        super(Voxel_Data, self).__init__(*args, **kwargs)

        self.mask = mask

        # if len(self.mask.shape) == 3:
        self.domain = ImageDomain(mask=self.mask, pixelsizes=pixelsizes, verbosity=self.verbosity)
    ...
    def make_coords(self):
        self.datatensor = numpy.zeros((len(self.time_filename_mapping), self.voxel_center_coordinates.shape[0], self.voxel_center_coordinates.shape[1] + 2))
        ...
        for key, filename in self.time_filename_mapping.items():
            ...
            self.measurements[key] = numpy.hstack((
                                                    numpy.zeros((self.voxel_center_coordinates.shape[0], 1)) + float(key),
    ...
    def sample(self, n: int) -> tuple:
        ...
        time_samples = numpy.random.randint(low=0, high=self.datatensor.shape[0], size=n)
        space_samples = numpy.random.randint(low=0, high=self.datatensor.shape[1], size=n)
...
class FEniCS_Data(MRI_Data):
    ...
    def __init__(self, function_space = None, mask: numpy.ndarray = None, *args, **kwargs) -> None:
        ...
        for key, filename in self.time_filename_mapping.items():
            ...
            if mask is None:
                ...
                print("Mask covers", format(mask.sum() * 100 / mask.size, ".0f"), r"% of the image voxels.")

                if numpy.allclose(mask.sum() / mask.size, 1) or mask.sum() == 0:
                    raise ValueError("Mask covers either no or all voxels, did you set all voxel values outside the brain to np.nan ?")

            self.measurements[key] = read_image(str(filename), functionspace=self.function_space, mask=mask)
