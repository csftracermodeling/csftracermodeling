...
def function_to_image(
    ...
    if mask is None:
        ...
        if fraction_of_image > 1 - 1e-10 and not allow_full_mask:
            raise ValueError(
                "The supplied mask covers the whole image so you are probably doing something wrong."
...
def make_coordinate_grid(image: np.ndarray, pixelsizes: tuple) -> np.ndarray:
    ...
    if len(image.shape) == 2:
        ...
        XX, YY, ZZ = np.meshgrid(
            coordinate_axis, coordinate_axis, coordinate_axis, indexing="ij"
        )
...
def init_collocation_points(coords, num_points, t_max, t_min):
    ...
    print(
        "Initialized collocation points with mean t = ",
        format(np.mean(t).item(), ".2f"),
        ", min t = ",
        format(np.min(t).item(), ".2f"),
        ", max t = ",
        format(np.max(t).item(), ".2f"),
    )
...
def largest_cc(mask):
    ...
    print(
        f"Largest cc has {size} voxels of {np.prod(mask.shape)}".format(
...
def cut_to_box(image, mask=None, box_bounds=None):
    ...
    if len(image.shape) == 3:
        returnimage = image[
            xlim_box[0] : xlim_box[1],
            ylim_box[0] : ylim_box[1],
            zlim_box[0] : zlim_box[1],
        ]
    else:
